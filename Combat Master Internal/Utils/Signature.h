...
uint64_t* find_sig(std::string module_name, const std::string& byte_array)
{
	...
	if (it != modules.end()) {
		module = it->second;
		printf("Reusing module %s\n", module_name.c_str());
	}
	else {
		module = GetModuleHandleA(module_name.c_str());
		modules.insert(std::make_pair(module_name, module));
		printf("Emplaced module %s for the first time\n", module_name.c_str());
	}
	// The problem was here: a missing semicolon.
	// The compiler read the 'if' on the next line as a continuation of the 'printf' statement, causing the syntax error.
